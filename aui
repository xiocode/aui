...
case "$1" in
  ...
  configure_hostname(){
    ...
    arch-chroot $MOUNTPOINT /bin/bash -c "sed -i '/127.0.0.1/s/$/ '${HN}'/' /etc/hosts"
    arch-chroot $MOUNTPOINT /bin/bash -c "sed -i '/::1/s/$/ '${HN}'/' /etc/hosts"
  }
  #}}}
  #CONFIGURE TIMEZONE {{{
  configure_timezone(){
        ...
        if contains_element "$ZONE" "${zone[@]}"; then
          ...
          select SUBZONE in "${subzone[@]}"; do
            if contains_element "$SUBZONE" "${subzone[@]}"; then
              arch-chroot $MOUNTPOINT /bin/bash -c "ln -s /usr/share/zoneinfo/${ZONE}/${SUBZONE} /etc/localtime"
              break
            else
              invalid_option
            fi
    ...
    arch-chroot $MOUNTPOINT /bin/bash -c "echo ${ZONE}/${SUBZONE} > /etc/timezone"
  }
  #}}}
  ...
  configure_hardwareclock(){
    ...
    select OPT in "${hwclock_list[@]}"; do
      if contains_element "$OPT" "${hwclock_list[@]}"; then
        arch-chroot $MOUNTPOINT /bin/bash -c "hwclock --systohc --${OPT}";
        break
      else
        invalid_option
      fi
  ...
  configure_locale(){
    ...
    arch-chroot $MOUNTPOINT /bin/bash -c "sed -i '/'$LOCALE'/s/^#//' /etc/locale.gen"
  ...
  configure_bootloader(){
    case $BOOTLOADER in
      Grub2)
        ...
        arch-chroot $MOUNTPOINT /bin/bash -c "modprobe dm-mod"
        if [[ $UEFI -eq 1 ]]; then
          arch-chroot $MOUNTPOINT /bin/bash -c "grub-install --target=x86_64-efi --efi-directory=/boot/efi --bootloader-id=arch_grub --recheck"
        else
          arch-chroot $MOUNTPOINT /bin/bash -c "grub-install --target=i386-pc --boot-directory=/boot --recheck ${ROOT_DEVICE}"
        fi
        arch-chroot $MOUNTPOINT /bin/bash -c "mkdir -p /boot/grub/locale"
        arch-chroot $MOUNTPOINT /bin/bash -c "cp /usr/share/locale/en\@quot/LC_MESSAGES/grub.mo /boot/grub/locale/en.mo"
      ...
      Syslinux)
        ...
        select OPT in "${syslinux_install_mode[@]}"; do
          case "$REPLY" in
            1)
              arch-chroot $MOUNTPOINT /bin/bash -c "syslinux-install_update -i -a -m"
              break
              ;;
            2)
  ...
  *)
  ...
  select_user(){
    ...
    configure_user_account(){
      ...
      echo "export EDITOR=\"$EDITOR\"" >> /home/${USER_NAME}/.bashrc
      #}}}
  ...
  add_custom_repositories(){
    ...
    if [[ $OPTION == y ]]; then
      ...
      ncecho " ${BBlue}[${White}${BWhite}X${White}${BBlue}]${White} Syncing repositories "
      pacman -Syy >>"$LOG" 2>&1 &
      pid=$!;progress $pid
      pause_function
    fi
  ...
  install_basic_setup(){
    ...
    system_ctl enable cronie.service
    ...
    system_ctl enable ntpd.service
    pause_function
  ...
  install_lmt(){
    ...
    if [[ $OPTION == y ]]; then
      ...
      system_ctl enable laptop-mode.service
      pause_function
    fi
  }
  #}}}
  ...
  install_video_cards(){
    ...
    #Nvidia
      ...
      is_package_installed "nouveau-dri" && pacman package_remove "nouveau-dri"
      package_install "pangox-compat" #fix nvidia-settings open
        ...
        for OPT in ${OPTIONS[@]}; do
          case "$OPT" in
            ...
            3)
              ...
              replaceinfile '#greeter-session=.*$' 'greeter-session=lightdm-gtk-greeter' /etc/lightdm/lightdm.conf
        ...
        select OPT in "${kde_install[@]}"; do
          case "$REPLY" in
            1)
              ...
              ;;
            2)
              package_install "kdebase phonon-gstreamer kde-l10n-$LANGUAGE_KDE"
              break
              ;;
            *)
              invalid_option
              ...
              for OPT in ${OPTIONS[@]}; do
                case "$OPT" in
                  ...
                  2)
                    ...
                    ;;
                  3)
                    aur_package_install "android-sdk android-apktool android-sdk-platform-tools android-udev eclipse-android"
                    groupadd android
              ...
              for OPT in ${OPTIONS[@]}; do
                case "$OPT" in
                  ...
                  8)
                    ...
                    ;;
                  9)
                    package_install "rhythmbox libdmapsharing"
